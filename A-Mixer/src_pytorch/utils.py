...
def data_masks(all_usr_pois, item_tail, maxl=200):
    ...
    len_max = max(us_lens)
    us_pois = [upois + item_tail * (len_max - le + 1) for upois, le in zip(all_usr_pois, us_lens)] # padding with item_tail to the right
...
class Data(Dataset):
    ...
    def get_slice(self, i):
        ...
        mask1 = torch.tensor(np.array(masks)).long()
        targets = torch.tensor(targets).long()
        n_node = torch.tensor(np.array(n_node)).long()
