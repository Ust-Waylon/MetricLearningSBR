...
class DIDN(nn.Module):
    ...
    def __init__(self, n_items, hidden_size, embedding_dim, batch_size, max_len, position_embed_dim, alpha1,  alpha2, alpha3, pos_num,neighbor_num, n_layers=1, logit_type='dot', scale=1.0):
    ...
    def forward(self, seq, lengths):
        ...
        if self.logit_type == "euclidean":
            # negative euclidean distance between session embeddings and item embeddings
            scores = -torch.cdist(sess_final, item_embs, p=2) * self.scale
        elif self.logit_type == "dot":
            # dot product between session embeddings and item embeddings (original)
            scores = torch.matmul(sess_final, item_embs.permute(1, 0))
        elif self.logit_type == "cos":
            # cosine similarity between session embeddings and item embeddings
            sess_final = F.normalize(sess_final, p=2, dim=1)
            item_embs = F.normalize(item_embs, p=2, dim=1)
            scores = torch.matmul(sess_final, item_embs.permute(1, 0)) * self.scale
        else:
            raise ValueError(f"Invalid logit type: {self.logit_type}")

        return scores
    
    def get_session_emb(self, seq, lengths):
        seq = seq.transpose(0, 1) # added to make the code compatible with the dataloader
    ...
    def moe_forward(self, seq, lengths):
        sess_final = self.get_session_emb(seq, lengths)
        ...
        output_probs, expert_probs, expert_weights = self.moe_decoder(sess_final, lengths, item_embs)
        return output_probs, expert_probs, expert_weights
