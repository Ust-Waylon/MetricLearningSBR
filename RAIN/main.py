...
parser.add_argument('--dataset', type=str,default='tmall',help='dataset name: retailrocket/lastfm')
...
parser.add_argument('--logit_type', type=str, default='dot', help='logit type: dot, euclidean')
...
def main():
    ...
    item_count = pickle.load(open('datasets/' + opt.dataset + '/item_count.txt', 'rb'))
    n_node = max(item_count.keys()) + 1
    print(f'Loaded n_node = {n_node} from item_count.txt')
    ...
    model = trans_to_cuda(
        RAIN(adjacency=train_d.adjacency, 
             adjacency_a=train_d.adjacency_a, 
             adjacency_1=train_d.adjacency_1,
             ad1r=train_d.ad1r, 
             ad1c=train_d.ad1c, 
             n_node=n_node, 
             lr=opt.lr, 
